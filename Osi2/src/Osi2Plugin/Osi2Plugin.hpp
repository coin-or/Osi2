...
/*! \brief Unique ID for a plugin library
  ...
  It must be returned by the plugin library in the RegistrationParam block
  when the plugin \link Osi2::RegisterFunc registers an API \endlink.
...
/*! \defgroup PluginFuncs Plugin Management Functions
  ...
  The functions Osi2::RegisterFunc and Osi2::InvokeServiceFunc are defined by
  the plugin manager and passed to the plugin in the PlatformServices
  parameter of the \link Osi2::InitFunc initialisation function \endlink.
  
  All other functions must be defined by any plugin library, even if they
  do nothing.
*/
//@{
...
/*! \brief Function invoked by the plugin to register an API.
  ...
  part of the execution of the Osi2::InitFunc. It can also be invoked at any
  time while the plugin is active to register new APIs.
...
/*! \brief Type definition of the plugin exit function
  ...
  function must be returned as the return value of the Osi2::InitFunc.
...
/*! \brief Type definition of the \c initPlugin function
  ...
  Note the return type is Osi2::ExitFunc (used by PluginManager to tell
  plugins to cleanup). If the initialization failed for any reason the
  plugin must return NULL. It may also (optionally) report the error via
  the error reporting function provided in the PlatformServices parameter.
